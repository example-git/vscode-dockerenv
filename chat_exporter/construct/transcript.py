...
class Transcript_text(TranscriptDAO):
    async def export(self):
        if not self.messages:
            self.messages = [message async for message in self.channel.history(
                limit=self.limit,
        ...
        for msg in self.messages:
            ...
            if msg.content is not None:
                if "\r\n" in msg.content:
                    m = str(msg.content).split('\r\n')
                    for i in m:
                        if len(i) > 1:
                            message_text += str(f'{msg.author.display_name}: {i}\r\n')
                else:
                    if len(msg.content) > 1:
                        message_text += str(f'{msg.author.display_name}: {msg.content}\r\n')
        try:
            return message_text
        except Exception:            
