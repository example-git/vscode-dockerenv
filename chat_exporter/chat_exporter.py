...
from chat_exporter.construct.transcript import Transcript, Transcript_text
...
async def quick_export(
    ...
    pathlib.Path(f'./channeltranscripts/transcript-{channel.name}.html').write_bytes(io.BytesIO(transcript.encode()).getbuffer().tobytes())
...
async def quick_text(
    ...
    transcript = await Transcript_text(
        channel=channel,
        limit=None,
        messages=None,
        pytz_timezone="UTC",
    ...
    pathlib.Path(f'./channeltranscripts/transcript-{channel.name}.txt').write_bytes(io.BytesIO(transcript.encode()).getbuffer().tobytes())
